...
class Process(object):
        ...
        for self.certainDataSet in self.dataSets:
            ...
            with tqdm(total=len(self.trainImgName) + len(self.valImgName), desc=f'{sign}', ncols=60) as bar:
                ...
                for name in self.trainImgName:
                    ...
                    self.certainData = {'image': image, 'label': label, 'vessel': vessel}
                    self.__adjust()
    ...
    def __del(self):
        ...
        for item in ['train', 'test', 'validation']:
            ...
            for name in _dir:
                os.makedirs(self.dataPath + item + '/' + name)
